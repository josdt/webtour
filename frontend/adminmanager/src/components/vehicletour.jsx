...
const PlaceTour = () => {
    ...
    const handleSelectionVehicle = (event) => {
        console.log(event.target.value)
        setIdVehicle(event.target.value)
    }


    const urlvehicletour = "http://localhost:8080/travel/vehicletour"
    const [vehicletours, setVehicleTours] = useState([])

    const submitGetVehicleTour = (e) => {
        e.preventDefault();
        axios.get(urlvehicletour + '/' + idTour)
            .then(response =>
    ...
    const [vehicletour, setVehicleTour] = useState(init)
    const submitCreateVehicleTour = (e) => {
        e.preventDefault();
        axios.post(urlvehicletour, vehicletour,{ headers: { 'id': sessionStorage.getItem("idEmp") } })
            .then((response) =>{
                if (response.data === "" || response.data === null || response.data === undefined) {
                  setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
                } else {
                  console.log(response)
                  window.location = ("/vehicletour")
                }
              }
            )
            .catch(function (error) {
    ...
    const handleDataVehicleTour = (event) => {
        const newData = { ...vehicletour }
        newData[event.target.name] = event.target.value
        setVehicleTour(newData)
    }



    const submitDelete = (idtour, idvehicle) => {
        axios.delete(urlvehicletour + '/' + idtour + '/' + idvehicle,{ headers: { 'id': sessionStorage.getItem("idEmp") } })
            .then((response) =>{
                if (response.data === "" || response.data === null || response.data === undefined) {
                  setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
                } else {
            ...
            <div className="d-flex flex-column" id="content-wrapper">
                ...
                <div className="container-fluid"><button className="btn btn-link d-md-none rounded-circle mr-3" id="sidebarToggleTop" type="button"><i className="fas fa-bars" /></button>
                    ...
                    <div className="container-fluid">
                        <h3 className="text-dark mb-4">Quản lý danh mục phương tiện tour</h3>
                        <form className="form-row" onSubmit={submitCreateVehicleTour}>
                            <div className="form-group col-md-6">
                                <label className="col-md-2" htmlFor="formGroupExampleInput"><strong>Tour:</strong></label>
                                <select className="col-md-6 form-control form-control-sm custom-select custom-select-sm" required name='idTour' value={vehicletour.idTour} onChange={handleDataVehicleTour}>
                            ...
                            <div className="form-group col-md-6">
                                <label className="col-md-4" htmlFor="formGroupExampleInput"><strong>Phương tiện:</strong></label>
                                <select className="col-md-6 form-control form-control-sm custom-select custom-select-sm" required name='idVehicle' value={vehicletour.idVehicle} onChange={handleDataVehicleTour}>
                                    <option disabled value={''}>Chọn phương tiện...</option>
                                    {vehicles.map(vehicle =>
                                        <option value={vehicle.idVehicle}>{vehicle.idVehicle}</option>
                                    )}
                                </select>
                            </div>
                ...
                : null
                        ...
                        <div className="card shadow">
                            <div className="card-header py-3">
                                <p className="text-primary m-0 font-weight-bold">Thông tin phương tiện tour</p>
                            </div>
                            <div className="card-body">
                                <form onSubmit={submitGetVehicleTour} id="dataTable_length" className="form-row">
                                    <label className="col-md-1"><strong>Chọn tour</strong></label>
                                ...
                                <div className="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                                    <table className="table my-0" id="dataTable">
                                        ...
                                        <tbody>
                                            {vehicletours.map(vehicletour =>
                                                <tr key={vehicletour.id}>
                                                    <td>{vehicletour.idTour}</td>
                                                    <td>{vehicletour.idVehicle}</td>
                                                    <td><button class="btn btn-danger" disabled={checkrole(sessionStorage.getItem("role"))} onClick={() => submitDelete(vehicletour.idTour, vehicletour.idVehicle)} >Xoá</button></td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th><strong>Mã phương tiện</strong></th>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
