...
const Place = () => {
  ...
  const [place, setPlace] = useState(init)
  const submitCreatePlace = (e) => {
    e.preventDefault();
    axios.post(urlplace, place, { headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) => {
  ...
  const submitDelete = (id) => {
    axios.delete(urlplace + '/' + id, { headers: {'id': sessionStorage.getItem("idEmp") }})
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
  ...
  function edit(place) {
    document.getElementById("idPlace").value = place.idPlace;
  ...
  function modify() {
    ...
    axios.put(urlplace, place, { headers: {'id': sessionStorage.getItem("idEmp") }})
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
  ...
  return (
    <div id="wrapper">
      ...
      <div className="d-flex flex-column" id="content-wrapper">
        <div id="content">
          ...
          <div className="container-fluid">
            <h3 className="text-dark mb-4">Quản lý danh mục địa danh</h3>
            <form className="form-row" onSubmit={submitCreatePlace}>
              <div className="form-group col-md-4">
                <label htmlFor="formGroupExampleInput">Mã địa danh</label>
                <input type="text" className="form-control" id="formGroupExampleInput" placeholder="nhập mã địa danh" required name='idPlace' value={place.idPlace} onChange={handleDataPlace} />
              </div>
              <div className="form-group col-md-4">
                <label htmlFor="formGroupExampleInput">Tên địa danh</label>
                <input type="text" className="form-control" id="formGroupExampleInput" placeholder="nhập tên địa danh" required name='name' value={place.name} onChange={handleDataPlace} />
              </div>
              <div className="form-group col-md-4">
            ...
            <div className="card shadow">
              <div className="card-header py-3">
                <p className="text-primary m-0 font-weight-bold">Thông tin địa danh</p>
              </div>
                    ...
                    <tbody>
                      {places.map(place =>
                        <tr key={place.idPlace}>
                          <td>{place.idPlace}</td>
                          <td>{place.name}</td>
                          <td><img className="rounded-circle mr-2" width={30} height={30} src={`assets/img/${place.image}`} /></td>
                          <td><button type="button" disabled={checkrole(sessionStorage.getItem("role"))} class="btn btn-secondary" data-toggle="modal" data-target="#popupPlace" onClick={() => edit(place)}>Sửa</button></td>
                          <td><button class="btn btn-danger" disabled={checkrole(sessionStorage.getItem("role"))} onClick={() => submitDelete(place.idPlace)}  >Xoá</button></td>
                        </tr>
                      )}
                    </tbody>
                    <tfoot>
                      <tr>
                        ...
                        <td><strong>Tên địa danh</strong></td>
      ...
      <div className="modal fade" id="popupPlace" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            ...
            <div className="modal-body">
              <form>
                <div className="form-group">
                  <label htmlFor="idPlace">Mã địa danh</label>
                  <input type="text" className="form-control" id="idPlace" placeholder="nhập mã địa danh" required name='idPlace' disabled />
                </div>
                <div className="form-group">
