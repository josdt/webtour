...
const submitCreateService=(e)=>{
  e.preventDefault();
  axios.post(urlservice,service, { headers: { 'id': sessionStorage.getItem("idEmp") } })
  .then((response) =>{
  ...
  const submitDelete = (id) => {
    axios.delete(urlservice + '/' + id, { headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
  ...
  function edit(service){
    document.getElementById("idService").value = service.idService;
  ...
  function modify(){
    ...
    axios.put(urlservice, service, { headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
...
const checkrole = (role) =>{
  ...
  return (
    ...
    >
      ...
      <div className="d-flex flex-column" id="content-wrapper">
        ...
        <nav className="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
          ...
          <div className="container-fluid">
            <h3 className="text-dark mb-4">Quản lý danh mục dịch vụ</h3>
            <form className="form-row" onSubmit={submitCreateService}>
              <div className="form-group col-md-6">
                <label htmlFor="formGroupExampleInput">Mã dịch vụ</label>
                <input type="text" className="form-control" id="formGroupExampleInput" required placeholder="nhập mã dịch vụ" name='idService' value={service.idService} onChange={handleDataService} />
              </div>
              <div className="form-group col-md-6">
                <label htmlFor="formGroupExampleInput">Tên dịch vụ</label>
                <input type="text" className="form-control" id="formGroupExampleInput" required placeholder="nhập tên dịch vụ" name='name' value={service.name} onChange={handleDataService} />
              </div>
            ...
            <div className="card shadow">
              <div className="card-header py-3">
                <p className="text-primary m-0 font-weight-bold">Thông tin dịch vụ</p>
              </div>
                    ...
                    <tbody>
                      {services.map(service =>
                        <tr key={service.idService}>
                          ...
                          <td>{service.name}</td>
                          <td><button class="btn btn-secondary" disabled={checkrole(sessionStorage.getItem("role"))} data-toggle="modal" data-target="#popupService" onClick={() =>edit(service)}>Sửa</button></td>
                          <td><button class="btn btn-danger" disabled={checkrole(sessionStorage.getItem("role"))} onClick={() => submitDelete(service.idService)} >Xoá</button></td>
                        </tr>
                      )}
      ...
      <div className="modal fade" id="popupService" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLongTitle">Sửa dịch vụ</h5>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                  <label htmlFor="idService">Mã dịch vụ</label>
                  <input type="text" className="form-control" id="idService" placeholder="nhập mã dịch vụ" required name='idService' disabled />
                </div>
                <div className="form-group">
                  <label htmlFor="name">Tên dịch vụ</label>
                  <input type="text" className="form-control" id="name" placeholder="nhập tên dịch vụ" required name='name'  />
                </div>
