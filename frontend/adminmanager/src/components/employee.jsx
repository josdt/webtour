...
const Employee = () => {
  ...
  const submitCreateEmployee = (e) => {
    e.preventDefault();
    axios.post(urlemployee, employee,{ headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) =>{
  ...
  const handleDataEmployee = (event) => {
  ...
  const submitDelete = (id) => {
    axios.delete(urlemployee + '/' + id,{ headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
  ...
  function edit(employee) {
    document.getElementById("idEmployee").value = employee.idEmployee;
    document.getElementById("name").value = employee.name;
    document.getElementById("address").value = employee.address;
    document.getElementById("email").value = employee.email;
    document.getElementById("phone").value = employee.address;
    document.getElementById("password").value = employee.email;
  }
  ...
  function modify() {
    ...
    axios.put(urlemployee, employee, { headers: { 'id': sessionStorage.getItem("idEmp") } })
      .then((response) =>{
        if (response.data === "" || response.data === null || response.data === undefined) {
          setError("Cảnh báo: Bạn không phải là quản lý, không có quyền chỉnh sửa dữ liệu!")
        } else {
...
const checkrole = (role) =>{
  ...
  return (
    <div id="wrapper">
      ...
      <div className="d-flex flex-column" id="content-wrapper">
        ...
        <nav className="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
          ...
          <div className="container-fluid">
            <h3 className="text-dark mb-4">Quản lý danh mục nhân viên</h3>
            <div>
              <form onSubmit={submitCreateEmployee}>
                <div className="form-row" >
                  <div className="form-group col-md-3">
                    <label htmlFor="inputEmail4">Mã nhân viên</label>
                    <input type="text" className="form-control" id="inputEmail4" required placeholder="nhập mã nhân viên" name='idEmployee' value={employee.idEmployee} onChange={handleDataEmployee} />
                  </div>
                  <div className="form-group col-md-4">
                    <label htmlFor="inputPassword4">Họ tên</label>
                    <input type="text" className="form-control" id="inputPassword4" required placeholder="nhập họ tên nhân viên" name='name' value={employee.name} onChange={handleDataEmployee} />
                  </div>
                  <div className="form-group col-md-2">
                    <label htmlFor="inputPassword4">Quyền</label>
                    <select className="form-control" name='role' required value={employee.role} onChange={handleDataEmployee}>
                ...
                <div className="form-row">
                  <div className="form-group col-md-3">
                    ...
                    <input type="text" className="form-control" id="inputEmail4" required placeholder="nhập địa chỉ" name='address' value={employee.address} onChange={handleDataEmployee} />
                  </div>
                  <div className="form-group col-md-3">
                    <label htmlFor="inputPassword4">Email</label>
                    <input type="email" className="form-control" id="inputPassword4" required placeholder="nhập email" name='email' value={employee.email} onChange={handleDataEmployee} />
                  </div>
                  <div className="form-group col-md-3">
                    <label htmlFor="inputEmail4">Điện thoại</label>
                    <input type="text" className="form-control" id="inputEmail4" required placeholder="nhập số điện thoại" name='phone' value={employee.phone} onChange={handleDataEmployee} />
                  </div>
                  <div className="form-group col-md-3">
                    <label htmlFor="inputPassword4">Password</label>
                    <input type="password" className="form-control" id="inputPassword4" required placeholder="nhập password" name='password' value={employee.password} onChange={handleDataEmployee} />
                  </div>
                </div>
                <button disabled={checkrole(sessionStorage.getItem("role"))} className="btn btn-primary btn-block">Lưu</button>
              </form>
              {
              error.length > 0 ?
            ...
            <div className="card shadow">
              <div className="card-header py-3">
                <p className="text-primary m-0 font-weight-bold">Thông tin nhân viên</p>
              </div>
                    ...
                    <tbody>
                      {employees.map(employee =>
                        <tr key={employee.idEmployee}>
                          ...
                          <td>{employee.role}</td>
                          <td><button type="button" disabled={checkrole(sessionStorage.getItem("role"))} class="btn btn-secondary" data-toggle="modal" data-target="#popupEmployee" onClick={() => edit(employee)}>Sửa</button></td>
                          <td><button class="btn btn-danger" disabled={checkrole(sessionStorage.getItem("role"))} onClick={() => submitDelete(employee.idEmployee)} >Xoá</button></td>
                        </tr>
                      )}
      ...
      <div className="modal fade" id="popupEmployee" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLongTitle">Sửa nhân viên</h5>
            ...
            <div className="modal-body">
              <form>
                <div className="form-group">
                  ...
                  <input type="text" className="form-control" id="idEmployee" placeholder="nhập mã nhân viên" required name='idEmployee' disabled />
                </div>
                <div className="form-group">
                  <label htmlFor="name">Tên nhân viên</label>
                  <input type="text" className="form-control" id="name" placeholder="nhập tên nhân viên" required name='name' />
                </div>
                <div className="form-group">
                  <label htmlFor="address">Địa chỉ</label>
                  <input type="text" className="form-control" id="address" placeholder="nhập địa chỉ" required name='address' />
                </div>
                <div className="form-group">
                  <label htmlFor="email">Email</label>
                  <input type="text" className="form-control" id="email" placeholder="nhập email" required name='email' />
                </div>
                <div className="form-group">
                  <label htmlFor="phone">Điện thoại</label>
                  <input type="text" className="form-control" id="phone"  required name='phone'  disabled/>
                </div>
                <div className="form-group">
                  <label htmlFor="password">Mật khẩu</label>
                  <input type="text" className="form-control" id="password"  required name='password' disabled/>
                </div>
